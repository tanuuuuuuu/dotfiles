...
新規登録で $25 の無料クレジットが付与されるが、コンソールの Billing セクションで有効化が必要な場合がある。API 呼び出し時に `doesn't have any credits` エラーが出たら Billing を確認する。
...
$25 の無料クレジットで **数百〜数千回** の検索が目安（クエリの複雑さによる）。
